$(document).ready(function () {
    ...
    DAILYKPI2 = {
        filter: function (start, end) {
            var url = clientHTTPConfig.appContextRoot + "/dataaccess/dailykpi2";
            //console.log('dailyKpi2.js:start:', start, ' end:', end);
...
//								d.traffic_date = moment(d.traffic_date);
                ...
                var medianLoss = dc.barChart("#" + reportType + "medianLoss-chart", "dailyKpi2" + reportType);
                var medianLossPieChart = dc.pieChart("#" + reportType + "medianLoss-pieChart", "dailyKpi2" + reportType);
                var newDetections = dc.barChart("#" + reportType + "newDetections-chart", "dailyKpi2" + reportType);
                var newDetectionsPieChart = dc.pieChart("#" + reportType + "newDetections-pieChart", "dailyKpi2" + reportType);
                ...
                datatable = $("#" + reportType + "dailyKpi-datatable2").dataTable({
                    "bPaginate": true,
                    "colReorder": true,
                    "bLengthChange": true,
                    "sScrollX": "100%",
                    ...
                    "aoColumns": [ //TODO: may need to change when switching to Hive: 33 columns on UI, 33 used columns here, 37 columns in
                         ...
                         */, {
                            ...
                            "mRender": function (data, type, row) {
                                //debug
                                return "<a href='javascript:void(0);' onclick=\"DAIlYKPI2POP.popDect(" + data + "," + row.traffic_date.format('YYYYMMDD') + ",'GK');\" class='colLnk'>" + data + '</a>';
                            }
                        }, {
                            "mData": "gkc_new_count",
                            "sDefaultContent": "",
                            "mRender": function (data, type, row) {
                                return "<a href='javascript:void(0);' onclick=\"DAIlYKPI2POP.popDect(" + data + "," + row.traffic_date.format('YYYYMMDD') + ",'GKC');\" class='colLnk'>" + data + '</a>';
                            }
                        }, {
                            "mData": "tcg_new_count",
                            "sDefaultContent": "",
                            "mRender": function (data, type, row) {
                                return "<a href='javascript:void(0);' onclick=\"DAIlYKPI2POP.popDect(" + data + "," + row.traffic_date.format('YYYYMMDD') + ",'TCG');\" class='colLnk'>" + data + '</a>';
                            }
                        }, {
                            ...
                            "sDefaultContent": "",
                            "mRender": function (data, type, row) {
                                return "<a href='javascript:void(0);' onclick=\"DAIlYKPI2POP.popDect(" + data + "," + row.traffic_date.format('YYYYMMDD') + ",'TOTAL');\" class='colLnk'>" + data + '</a>';
                            }
                        }, {
                ...
                function RefreshTable() {
                    dc.events.trigger(function () {
                        logger.debug("Calling REFRESHTABLE in dailykpi2.js");
